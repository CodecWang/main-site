...
Easel 是 Arc 自带的类似白板的功能，Note 是笔记功能。我其实比较反感浏览器加这些非本职工作的功能，并且它的使用和协同都只限 Arc 浏览器。另外这两个功能我都有更好更通用的工具，笔记我用的是 Notion，白板用的 Miro，所以我个人是没咋用这两个，这里大致体验下：
...
总结：已经好久没有让我眼前一亮或者用着很舒服的软件产品了。我目前用了一个月，Arc 基本没遇到过让我糟心的点，反而是不断发现一些小功能让我会心一笑。但目前它的 IOS 移动端做的确实很差，对于有数据同步和移动端需求的用户并不友好。**所以如果你能习惯垂直标签且在大屏设备上使用，那我是强烈推荐的，否则依然首推 Edge。**
